...
const StoryCard = memo(function StoryCard({ story }) {
  // Pre-determine status styling to avoid conditional evaluations during rendering
  const statusStyles = 
    ...
    <div className="relative h-80 rounded-lg overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-3">
        ...
        <div className="mb-2 flex items-center justify-between text-xs">
          ...
          <span className={`rounded-md px-2 py-1 text-xs capitalize ${statusStyles}`}>
            {story.status}
          </span>
        </div>
